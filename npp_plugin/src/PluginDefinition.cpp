...
void build()
{
	...
	cmd	<< _T("g++ -x c -o\"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".iprepr\" -E \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".ic\"");
	...
	cmd	<< _T("industrialc -o \"") // note the space after -o is important
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".cpp\" \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".iprepr\"");
	thePlugin.CreateChildProcess(&exit_code, cmd.str().c_str());
	...
	//Build
	//avr-gcc -mmcu=atmega168 -DF_CPU=16000000UL -o "<dir>/<file>.elf" "<dir>/<file>.cpp"
	cmd.str(_T(""));cmd.clear();
	cmd	<< _T("avr-gcc -mmcu=atmega168 -DF_CPU=16000000UL -o \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".elf\" \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".cpp\"");
	thePlugin.CreateChildProcess(&exit_code, cmd.str().c_str());
	...
	cmd	<< _T("avr-objcopy -O ihex -R .eeprom  \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".elf\" \"") 
		<< dir.c_str() << _T("\\") << file.c_str() << _T(".hex\"");
	thePlugin.CreateChildProcess(&exit_code, cmd.str().c_str());
